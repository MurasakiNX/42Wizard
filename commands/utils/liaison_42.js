        ...
        switch (subcommand) {
            case 'informations': {
                ...
                const informationsEmbed = client.baseEmbed()
                    ...
                    .setDescription(`- Étapes:\n  - **Créez** un dossier \`.42Wizard\` (C'est un dossier caché, n'oubliez pas de les afficher) dans votre \`home\` et placez le code ci-dessous (script de détection) dans un fichier \`script.sh\` (N'oubliez pas de \`chmod +x\`).\n  - **Créez** un fichier \`userKey\` dans le dossier \`.42Wizard\` et inscrivez-y la clé d'identification.\n\n- **Exécutez** ce script avec ce que vous voulez (Minishell ???).\n*Si vous souhaitez automatiser son exécution, collez dans un terminal le script d'automatisation ci-dessous. Pour le rendre actif, reloggez-vous.*\n\n- Clé d'identification: \`${FortyTwoSyncDB.syncKey}\`\n- Script de détection:\n\`\`\`sh\n${readFileSync('./script.sh', 'utf8')}\`\`\`\n- Script de lancement automatique:\n\`\`\`sh\n${readFileSync('./autostart.sh', 'utf8')}\`\`\``)
